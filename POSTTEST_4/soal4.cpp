...
void sortedInsert(Node *&head_ref, int data)
{
    ...
    if (data < head_ref->data)
    {
        ...
        newNode->next = head_ref;    // newNode next ke head_ref
        newNode->prev = tail;        // newNode prev ke tail
        tail->next = newNode;        // Tail next ke newNode
        head_ref->prev = newNode;    // Head prev ke newNode
        head_ref = newNode;          // Update head_ref ke newNode
        return;
    }
    ...
    {
        current = current->next; // Geser ke next node
    }
    // 3. Setelah loop, sisipkan newNode setelah current
    // Sambungkan newNode di antara current dan current->next
    newNode->next = current->next;
    newNode->prev = current;
    // Sambungkan current->next ke newNode
